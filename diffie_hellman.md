...
They publish the public counterparts, A = g^a, B = g^b with some cyclic group with generator g which is agreed beforehand.
Now each can raise the others published number to their own secrets, Alice does B^a, Bob does A^b 
B^a = (g^b)^a = g^(ba)
A^b = (g^a)^b = g^(ab)
ab = ba 
Thus now they both know a number noone else can calculate from the public values and they can use it to enccrypt and decrypt messages. 
